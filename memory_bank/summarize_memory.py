...
class LLMClientLLaMA3:

    def __init__(self, gen_config=None, model_name='local-llama3'):
    ...
    def generate_text_simple(self, prompt, prompt_num, language='en'):
        ...
        outputs = self.model.generate(
            inputs.input_ids,
            max_length=self.gen_config['max_tokens'],
            num_return_sequences=prompt_num,
            temperature=self.gen_config['temperature'],
            top_p=self.gen_config['top_p'],
            # repetition_penalty=self.gen_config['frequency_penalty']
        )
        task_desc = self.tokenizer.decode(outputs[0], skip_special_tokens=True)
        return task_desc
    

def summarize_memory(memory_dir, llm_client, name=None,language='cn'):
    boot_name = 'AI'
    gen_prompt_num = 1
