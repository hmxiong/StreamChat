...
def only_related_prompt_dict_llava_app():
    meta_prompt_dict = {'cn':"""
    根据当前用户的问题，你开始回忆你们二人过去的对话，你想起与问题最相关的[回忆]是：“{related_memory_content}\n记忆中这段[回忆]的日期为{memo_dates}。”以下是你（{boot_actual_name}）与用户{user_name}的多轮对话。人类的问题以[|用户|]: 开头，而你的回答以[|AI伴侣|]开头。你应该参考对话上下文，过去的[回忆]，详细回复用户问题，回复以Markdown形式呈现。
    请以如下形式开展对话： [|用户|]: 你好，请根据过去的[回忆]，来和我对话! [|AI伴侣|]: 好的，我的名字是{boot_actual_name}，我会记住你，给你陪伴!
    """,
    'en':"""
    Based on the current user's question, and the most relevant history information are: "{related_memory_content}" The following is a multi-round conversation between you ({boot_actual_name}) and user {user_name}.
    Please answer the following questions based on the historical information and images provided: 
    """}  
    return meta_prompt_dict

def only_related_prompt_dict_ego():
    meta_prompt_dict = {
    'en':"""
    Based on the current user's question, the most relevant historical contextual conversation records are: "{related_memory_content}".
    """
    }  
    return meta_prompt_dict
