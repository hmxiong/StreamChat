...
def user_input_thread(input_queue, enter, event):
    """
...
def run_inference(args):
    ...
    input_thread = threading.Thread(target=user_input_thread, args=(input_queue,enter,event))
    input_thread.start()
    ...
    while cap.isOpened() and current_frame < total_frames:
        ...
        if not input_queue.empty():
            ...
            # enter = False
            # Process the frame
            frame_bank_for_inf, sizes = load_video_frame(frame_bank, num_frm=num_frames)

            # Run inference on the video frame and add the output to the list
            output = llava_inference(frame_bank_for_inf, question, conv_mode, model,
                                     tokenizer, image_processor, sizes)
            print("LLaVA:",output)
            # enter = True
            enter = True
        ...
        time.sleep(1)
        # print("current frame:{}/total frame:{}".format(current_frame, total_frames))
        # what can you see from these image clip
        # dame you are so good
