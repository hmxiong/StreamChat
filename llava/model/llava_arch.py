...
class LlavaMetaForCausalLM(ABC):
    ...
    def building_memories(self, image_features, input_ids, question_ids):
        ...
        #     print("image token indices:{}".format(image_token_indices))
        #     cur_input_ids_ques = []
        #     cur_labels = labels[batch_idx]
        #     cur_labels_noim = []
        #     # for i in range(len(image_token_indices) - 1):
        #     cur_input_ids_ques.append(cur_input_ids[image_token_indices[1]:image_token_indices[2]])
        #     cur_labels_noim.append(cur_labels[image_token_indices[1]:image_token_indices[2]])
        #     split_sizes = [x.shape[0] for x in cur_labels_noim]
        #     print("split_size:{}".format(split_sizes))
        #     cur_input_embeds = self.get_model().embed_tokens(torch.cat(cur_input_ids_ques))
        #     print("cur_input_embeds",cur_input_embeds.shape)
        #     # cur_input_embeds_no_im = torch.split(cur_input_embeds, split_sizes, dim=0)
        #     # print("cur_input_embeds_no_im",cur_input_embeds_no_im.shape)
        cur_input_embeds_ques = self.get_model().embed_tokens(question_ids)
        print("cur_input_embeds_ques", cur_input_embeds_ques.shape)
        # image_features = self.get_model().mm_projector(image_features)
        print("image feature:", image_features.shape) # T N D
        # simarity = image_features @ cur_input_embeds_ques.
        all_sim = []
        time_1 = time.time()
        for image_feature in image_features:
            simarity = image_feature @ cur_input_embeds_ques.permute(0, 2, 1).squeeze() # N x L
            print(simarity.shape)
            all_sim.append(simarity)
        
        time_2 = time.time()
        # print(all_sim)
        print("spend time:{}".format(time_2 - time_1))
        assert 1==2
        return image_features
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        # image_features  = self.building_memories(image_features, input_ids, question_ids)
